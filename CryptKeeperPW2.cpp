...
#include "CryptKeeperPW2.h"

// performs a SHA1 hash on a vector of unsigned chars, using OpenSSL SHA1 function
vector<unsigned char> CryptKeeperPW2::SHA1(vector<unsigned char> input)
{
...
vector<unsigned char> CryptKeeperPW2::HMAC_SHA1(vector<unsigned char> key, vector<unsigned char> message)
{
...
vector<unsigned char> CryptKeeperPW2::StretchKey(unsigned int length, unsigned int passes, string password, 
...
// salt, so initialize CryptKeeperAES with a blank key.
CryptKeeperPW2::CryptKeeperPW2(const char *pw) : CryptKeeperAES("00000000000000000000000000000000")
{
	password = pw;
}

CryptKeeperPW2::~CryptKeeperPW2()
{
}

bool CryptKeeperPW2::Open(const char *filename, const char *mode)
{
	...
	if(!CryptKeeperAES::Open(filename, mode)) return false;

	// now the nonce is available; take that and the password and generate an AES key from 
	// it (32 bytes)
	key = StretchKey(32, 4096, password, nonce);
